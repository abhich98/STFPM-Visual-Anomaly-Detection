...
def export_onnx(model: STFPM, config: dict[str, Any], checkpoint_path: str, output_path: str) -> str:
    ...
    image_size = int(config["dataset"].get("image_size", 256))
    ...
    export_cfg = config.get("onnx", {})
    dynamic = bool(export_cfg.get("dynamic_batch", True))
    ...
    torch.onnx.export(
        ...
        str(out_path),
        export_params=True,
        opset_version=int(export_cfg.get("opset", 17)),
